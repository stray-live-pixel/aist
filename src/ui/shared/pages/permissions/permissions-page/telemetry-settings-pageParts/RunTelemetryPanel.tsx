...
/**
 * Что это: карточки run/tool telemetry.
 * Зачем нужно: сохраняет существующую аналитику качества prompt/tools отдельно от performance telemetry.
 * Какую продуктовую проблему решает: команда видит стоимость и стабильность запусков без хранения prompt, args и outputs.
 */
export function RunTelemetryPanel({ telemetry }: { telemetry: AgentTelemetryDashboard }) {
  const { t } = useI18n();
