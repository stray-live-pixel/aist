...
export function dispatch(
  ...
  return dispatchDaemonRpcMethod({
    context: {
      call: (methodName, ...args) => this.callRpcHandler(methodName, ...args)
    },
    connection,
    method,
    params,
    createMethodNotFoundError: (methodName) =>
      new DaemonRpcError(-32601, 'method.notFound', `Method not found: ${methodName}`)
  });
}
