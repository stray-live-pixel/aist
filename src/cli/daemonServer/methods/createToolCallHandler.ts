...
export function createToolCallHandler(this: AistDaemonServer): AgentRuntimeToolCallHandler {
  return async (params) => {
    const runner = new ToolRunner({
      ...
      memory: {
        add: async (candidate) => {
          const chat = await this.requireChat(params.chat.id);
          return applyMemoryCandidate({
            candidate,
            memoryStore: this.memoryStore,
            modelClient: this.options.modelClient || this.createRoutingModelClient(),
      ...
      agentService: {
        spawn: (input) => this.spawnAgent(input)
      },
