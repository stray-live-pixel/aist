...
// prettier-ignore
import type { AutonomousExportFormat, AutonomousLaunchOptions, CreateAutonomousFlowInput } from '../../../core/processes/autonomous';
// prettier-ignore
...
export async function autonomousFlowCreate(
  this: AistDaemonServer,
  params: unknown
): Promise<DaemonAutonomousFlowCreateResult> {
  const input = requireRecord(params, 'autonomous.flow.create params');
  const flowInput = requireRecord(input.flow, 'flow');
  const flow: CreateAutonomousFlowInput = {
    id: requireString(flowInput, 'id'),
    title: optionalString(flowInput, 'title')
  };
  await this.autonomousBackend.createFlow(flow);
  return {
