...
export function callRpcHandler(this: AistDaemonServer, methodName: string, ...args: unknown[]): Promise<unknown> {
  const handler = (this as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>)[methodName];
  if (typeof handler !== 'function') {
    throw new DaemonRpcError(-32601, 'method.notFound', `Method not found: ${methodName}`);
  }
