...
import { createInMemoryIsolationChatRepository } from '../isolation/runtime/createInMemoryIsolationChatRepository';
...
export async function runIsolationAgent(
  ...
  const chatId = `isolation-${input.session.sessionId}`;
  const toolRegistry = new DefaultToolRegistry();
  const chatRepository = createInMemoryIsolationChatRepository({
    chatId,
    title: createIsolationChatTitle(input.session.prompt),
  ...
  const result = await runtime.ask(chatId, input.session.prompt);
  if (!result.accepted) {
  ...
  return { runId: result.runId, answer: chatRepository.getChat().lastAnswer };
}
...
function createIsolationChatTitle(prompt: string): string {
  const normalized = prompt.replace(/\s+/g, ' ').trim();
  return normalized.length > 80 ? `${normalized.slice(0, 77)}...` : normalized || 'Isolated agent task';
}
