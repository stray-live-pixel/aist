...
import type { AgentRun, Chat, ChatMessage, OpenRouterMessage, ToolApprovalDecision, ToolCall } from './types';
...
describe('ToolRunner', () => {
  ...
  it('stores full approved output separately from compact model-visible output', async () => {
    const context = createRunnerContext();
    const stdout = 'large stdout '.repeat(1000);
  ...
  it('compacts repeated large outputs independently', async () => {
    const context = createRunnerContext();
    const stdout = 'repeat-output '.repeat(1000);
    const runner = createRunner(context, {
      filesystem: {
        execute: vi.fn(async () => ({
          ok: true,
