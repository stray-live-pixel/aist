...
import { resolveNodeWorkspacePath } from './filesystemTools';
import { createToolError, toStructuredToolFailure } from './toolErrors';
import type { OpenRouterTool, ToolPermissionMode } from './types';
...
async function runNodeSkillToolImpl(input: {
  ...
  const maxOutputChars = clampNumber(input.args.maxOutputChars, 20000, 1000, 100000);
