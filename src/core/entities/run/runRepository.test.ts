...
import type { RuntimeToolCallSnapshot } from '../../shared/types/types';
import { globalWorkspaceRunsDir } from '../storage/storage';
...
describe('RunRepository', () => {
  it('creates, lists, updates, appends logs and restores runs after restart', async () => {
    ...
    const runRoot = path.join(globalWorkspaceRunsDir(workspaceRoot), run.id);
  ...
  it('updates run metadata atomically without rewriting append-only logs', async () => {
    ...
    const eventsPath = path.join(globalWorkspaceRunsDir(workspaceRoot), run.id, 'events.jsonl');
