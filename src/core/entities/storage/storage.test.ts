...
describe('core storage paths', () => {
  ...
  it('keeps personal workspace-scoped artifacts under the global root', () => {
    const tempDir = createTempDir();
    const workspaceRoot = path.join(tempDir, 'workspace');
    const homeDir = path.join(tempDir, 'home');
    const globalWorkspace = globalWorkspaceRoot(workspaceRoot, homeDir);

    expect(globalWorkspace).toContain(path.join(homeDir, '.aist-agent', 'workspaces'));
    expect(globalWorkspace).not.toContain(path.join(workspaceRoot, '.aist-agent'));
    expect(globalWorkspaceChatsDir(workspaceRoot, homeDir)).toBe(path.join(globalWorkspace, 'chats'));
    expect(globalWorkspaceRunsDir(workspaceRoot, homeDir)).toBe(path.join(globalWorkspace, 'runs'));
    expect(globalWorkspaceTelemetryDir(workspaceRoot, homeDir)).toBe(path.join(globalWorkspace, 'telemetry'));
    expect(globalWorkspaceAutonomousSessionsDir(workspaceRoot, homeDir)).toBe(
      path.join(globalWorkspace, 'autonomous', 'sessions')
    );
  });

  it('keeps project-shareable workspace artifacts under project .aist-agent', () => {
  ...
  it('keeps legacy personal path helpers as global-storage facades', () => {
    const workspaceRoot = path.join(createTempDir(), 'workspace');

    expect(workspaceChatsDir(workspaceRoot)).toBe(globalWorkspaceChatsDir(workspaceRoot));
    expect(workspaceRunsDir(workspaceRoot)).toBe(globalWorkspaceRunsDir(workspaceRoot));
    expect(workspaceTelemetryDir(workspaceRoot)).toBe(globalWorkspaceTelemetryDir(workspaceRoot));
    expect(workspaceAutonomousSessionsDir(workspaceRoot)).toBe(globalWorkspaceAutonomousSessionsDir(workspaceRoot));
  });
});
