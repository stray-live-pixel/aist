...
import type { EditorContextMode, OpenRouterMessage } from './types';
...
export function governModelContext(input: ContextGovernorInput): GovernedContext {
  ...
  const historyTail = selectHistoryTail(input.history, budgets);
  const recentToolSummaries = collectRecentToolSummaries(input.history, budgets.recentToolSummaries);
  const editorPack = buildGovernedEditorContext(input.prompt, taskType, input.editorContext, budgets);
  ...
  const messages = [...historyTail.messages, { role: 'user' as const, content: userContent }];
  ...
  return {
    ...
    userContent,
    contextNote: note,
    editorContextBlock: editorPack.block,
    recentToolSummaries,
    keptHistoryMessages: historyTail.messages.length,
    omittedHistoryMessages: historyTail.omitted
  };
